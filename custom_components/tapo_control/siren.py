...
async def async_setup_entry(
    ...
    async def setupEntities(entry):
        ...
        if not supports_manual_siren(entry.get("camData")):
            LOGGER.debug(
                "Skipping TapoSirenEntity: model %s has no manual-siren API",
                (entry.get("camData") or {}).get("basic_info", {}).get(
                    "device_model"
                ),
            )
            return sirens
...
class TapoSiren(TapoSirenEntity):
    def __init__(self, entry: dict, hass: HomeAssistant, config_entry):
        ...
        self.sirenType = resolve_siren_type(entry["camData"])
    ...
    async def async_turn_on(self, duration: int | None = None, **kwargs) -> None:
        ...
        try:
            result = await trigger_siren(
                self._hass, self._entry, True, sirenType=self.sirenType
            )
        except Exception:
    ...
    async def async_turn_off(self, send: bool | None = True, **kwargs) -> None:
        if send:
            try:
                await trigger_siren(
                    self._hass, self._entry, False, sirenType=self.sirenType
                )
            except Exception:
                self._attr_available = False
                raise
